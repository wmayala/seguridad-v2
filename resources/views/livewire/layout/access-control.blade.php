        ...
        <div class="row">
            <div class="flex flex-col justify-start items-center gap-2 mb-5">
                ...
                <x-text-input class="text-center"
                    ...
                    maxlength="10"
                    placeholder="DUI | Placa | #Beneficiario"
                    autofocus>
                </x-text-input>
        ...
        @if($result)
                    ...
                    <div class="row">
                        <div class="bg-teal-100 border-t-4 border-teal-500 rounded-b text-teal-900 px-4 py-3 shadow-md mx-40 my-3" role="alert">
                            <div class="flex justify-center items-center gap-3">
                                ...
                                <span class="font-bold text-2xl uppercase">Acceso activo</span>
                            </div>
                        </div>
                    </div>
                @else
                    <div class="row">
                        <div class="bg-red-100 border-t-4 border-red-500 rounded-b text-red-900 px-4 py-3 shadow-md mx-40 my-3" role="alert">
                            <div class="flex justify-center items-center gap-3">
                                ...
                                <span class="font-bold text-2xl uppercase">Acceso expirado</span>
                            </div>
                        </div>
                    </div>
                @endif
                <div class="row">
                    <div class="grid grid-cols-[25%_75%] w-3/4 mx-auto gap-5 mb-3">
                        ...
                        <div class="flex flex-col justify-center ml-20">
                            ...
                            <span class="text-xs uppercase">Institución</span>
                            <span class="text-lg font-bold uppercase p-1 mb-3">{{ $result->institution->name }}</span>
                            <span class="text-xs uppercase">Marca</span>
                            <span class="text-lg font-bold uppercase p-1 mb-3">{{ $result->brand }}</span>
                            <span class="text-xs uppercase">Color</span>
                            <span class="text-lg font-bold uppercase p-1 mb-3">{{ $result->color }}</span>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="grid grid-cols-3 w-3/4 mx-auto gap-5">
                        <div class="rounded-full bg-[#313846] text-white text-xl font-bold text-center p-2 mr-20">{{ $result->plate }}</div>
                        <div class="flex flex-col">
                            <span class="text-xs uppercase">Expedición</span>
                            <span class="text-2xl font-bold">{{ date('d-m-Y', strtotime($result->issueDate)) }}</span>
                        </div>
            ...
            @elseif($table == 'Beneficiary')
                ...
                <div class="row">
                    <div class="grid grid-cols-[25%_75%] w-3/4 mx-auto gap-5 mb-3">
                        <div class=" flex items-center justify-center">
                            <img src="{{ asset('storage/' . $result->photo) }}" alt="" class="max-w-full max-h-full">
                            {{-- @if($result->photo)
                                <img src="{{ Storage::disk('s3')->url($result->photo) }}" alt="" class="max-w-full max-h-full">
                            @else
                                <span class="text-sm text-gray-400">Sin Foto</span>
                        ...
                        <div class="flex flex-col justify-center ml-20">
                            <span class="text-xs uppercase">Nombre de beneficiario</span>
                            <span class="text-lg font-bold uppercase p-1 mb-3">{{ $result->name }}</span>
                            <span class="text-xs uppercase">Parentesco</span>
                            <span class="text-lg font-bold uppercase p-1 mb-3">{{ $result->relationship }}</span>
                            <span class="text-xs uppercase">Empleado</span>
                            <span class="text-lg font-bold uppercase p-1 mb-3">{{ $result->empName }}</span>
                        </div>
                    </div>
                </div>
                <div class="row">
                    <div class="grid grid-cols-3 w-3/4 mx-auto gap-5">
                        <div class="rounded-full bg-[#313846] text-white text-xl font-bold text-center p-2 mr-20">{{ $result->record }}</div>
                        <div class="flex flex-col">
                            <span class="text-xs uppercase">Expedición</span>
                            <span class="text-2xl font-bold">{{ date('d-m-Y', strtotime($result->issueDate)) }}</span>
                        </div>
            ...
            @endif
            @if(Auth::user()->can('registrar-acceso') && \Carbon\Carbon::parse($result->expirationDate)->gt(today()))

                <div class="flex justify-center items-center gap-2">
                    <div class="flex justify-center mt-5 mb-5 p-2">
                        <button wire:click="confirmAccess()"
                            class="px-4 py-2 bg-black text-white text-sm font-semibold rounded-full hover:opacity-75 w-auto"
                        >
                            OK
                        </button>
                    </div>
